...
const ALL = [...boons, ...duos, ...hammers, ...aspects, ...keepsakeSlot, ...wares];

const KIND_LABEL = {
  boon: 'Boon', duo: 'Duo', hammer: 'Hammer', aspect: 'Aspect',
  keepsake: 'Keepsake', companion: 'Companion', ware: 'Ware',
};
          ...
          {results.map((item, i) => (
            <li key={item.id}>
              <button
                ...
                onClick={() => choose(item)}
              >
                <Image src={item.iconsrc} alt="" width={32} height={32} draggable={false} />
                <span className="quick-name">{item.title}</span>
                <span className="quick-kind">{KIND_LABEL[item.kind] ?? ''}</span>
              </button>
            </li>
          ))}
        </ul>
