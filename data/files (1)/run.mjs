...
const TARGETS = {
  ...
  hammers: () => {
    ...
    return { items, problems, notices, file: 'hammers.js',
             source: hammers.emit(items, WEAPONS.map(([w]) => w)) };
  },
  duo: () => {
...
for (const name of names) {
  ...
  const { items, problems = [], notices = [], file, source } = target();
  writeFileSync(join(OUT, file), source);

  const status = problems.length ? '!' : 'ok';
  console.log(`  ${status.padEnd(3)}${name.padEnd(12)} ${String(items.length).padStart(3)} items  ->  out/${file}`);
